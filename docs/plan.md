...
2. **VizChitra Sans** — turn that data into a real font with a real `RETA` axis.
3. **Delivery** — serve the font files, a browser-tested `fonts.css`, and usage docs.
...
- **Cairo's OFL carries no Reserved Font Name** — the copyright line is plain "Copyright 2009 The
  ...
  legal obligation. This materially de-risks Phase 2.
...
### The limitation that justifies Phase 2
...
run and keep kerning intact. The lab must surface this caveat, and Phase 2's acceptance test is
...
| 2   | **Slant only.** Per-glyph spacing and weight from LogoType are not modelled. Tracking stays a separate global CSS concern.                                                                                                                     |
| 3   | **Case-sensitive glyph keys.** `'V'` and `'v'` tune independently; uppercase seeded, lowercase at default.                                                                                                                                     |
| 4   | **Export + import JSON, persist to localStorage.** Tuning sessions must survive a reload and feed Phase 2.                                                                                                                                     |
...
| 7   | **Phase 2 = binary surgery with fontTools**, not a source build.                                                                                                                                                                               |
| 8   | **Verify Cairo's OFL Reserved Font Name, rename, ship OFL + attribution.**                                                                                                                                                                     |
| 9   | **Phase 3 = versioned immutable URLs + CORS + published `fonts.css`.**                                                                                                                                                                         |
...
| 13  | **Phase 2 operates on the full unsubsetted Cairo VF** from `google/fonts`; we do our own subsetting.                                                                                                                                           |
...
## 1.2 Pin the fonts properly — do this now, not in Phase 3
...
Pure and UI-free. This is the part that must survive into Phase 2, so it carries no view state.
...
kerning target for VIZCHITRA when Phase 2 checks that kerning survives at `RETA=1`.
...
same for Plex and Fira Code (both OFL) since Phase 3 redistributes them unmodified.

## 2.2 Build pipeline — `font-src/`
...
3. **Kerning across RETA** — confirm GPOS survives and decide whether kern values need variation at
   `RETA=1`. This is the whole point of the axis, so it is a gate, not a nicety.
...
## 2.3 Close the loop with the lab
...
# Phase 3 — fonts.vizchitra.com

Goal: one place that serves the font files, a browser-tested `fonts.css`, and the docs for consuming
them. Serves **VizChitra Sans** (ours), plus **IBM Plex Sans** and **Fira Code** unmodified.
...
## 3.2 Browser quirks — largely DONE in Phase 1
...
- **`type.vizchitra.com` for typography guidelines** — worth deciding in Phase 3 whether guidelines
...
from Cairo's `.glyphs`/UFO; changes to the sibling repos beyond the Phase 3 migration doc.
