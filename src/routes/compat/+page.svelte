<script lang="ts">
	...
	// Both generated by `pnpm test` (scripts/compat-matrix-reporter.mjs and
	// scripts/browser-versions.mjs) — not hand-maintained. AUTOMATED is keyed
	// by SLANT_TESTS id below and holds only ids slant.browser.test.ts has a
	// dedicated tagMatrix() call for; PLAYWRIGHT_VERSIONS is this repo's
	// pinned Playwright (package.json), queried directly via
	// `browser.version()` rather than assumed from the Playwright release
	// number.
	import AUTOMATED from '#lib/fonts/compat-matrix.generated.json';
	import PLAYWRIGHT_VERSIONS from '#lib/fonts/browser-versions.generated.json';
	...
	// Reads a SLANT_TESTS id's chromium/firefox/webkit verdict out of the
	// generated matrix (AUTOMATED, from slant.browser.test.ts's own tagMatrix()
	// calls) instead of it being retyped here by hand. Throws rather than
	// silently falling back to a stale/missing value — a missing entry means
	// either the id was renamed or `pnpm test` hasn't been run since a test
	// backing it was added, both of which should fail loudly, not render a
	// wrong badge.
	function automated(id: string): Pick<Verdict, 'chromium' | 'firefox' | 'webkit'> {
		const row = (
			AUTOMATED as Record<
				string,
				{ chromium: boolean | null; firefox: boolean | null; webkit: boolean | null }
			>
		)[id];
		if (!row || row.chromium === null || row.firefox === null || row.webkit === null) {
			throw new Error(
				`no complete automated matrix entry for '${id}' — run 'pnpm test' to regenerate compat-matrix.generated.json`
			);
		}
		return { chromium: row.chromium, firefox: row.firefox, webkit: row.webkit };
	}
	...
	const SLANT_TESTS: {
		...
		},
		{
			...
			klass: 'm-recommended',
			// Not in AUTOMATED: no test measures this exact combination
			// (bare oblique + explicit slnt against a range-free face) in
			// isolation — literal here, same as EXPLICIT_SLNT_ANYWHERE below.
			browsers: {
				chromium: true,
				firefox: true,
				webkit: true,
		...
		},
		{
			...
			klass: 'm-synthesis',
			// Not in AUTOMATED: this is a double-check a consumer runs on
			// their own page, not a claim slant.browser.test.ts measures
			// against a face with no italic-only sibling to compare against.
			browsers: {
				chromium: true,
				firefox: true,
				webkit: true,
